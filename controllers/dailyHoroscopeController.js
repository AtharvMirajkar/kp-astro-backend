...
export const getDailyHoroscope = async (req, res) => {
  ...
  const lang      = (req.query.language || req.query.lang || "en").toLowerCase().trim();
  const dateParam = req.query.date;

  if (!SUPPORTED_LANGS.includes(lang)) {
    return res.status(400).json({ success: false, message: `Unsupported language. Use: en, hi, mr` });
  }

  let targetDate;
  if (dateParam) {
    targetDate = new Date(dateParam + "T00:00:00.000Z");
    if (isNaN(targetDate)) return res.status(400).json({ success: false, message: "Invalid date. Use YYYY-MM-DD" });
  } else {
    targetDate = todayUTC();
  }
  ...
  try {
    ...
    if (!record) {
      return res.status(404).json({
        success: false, sign, date: toDateString(targetDate), language: lang,
        message: "Horoscope not available for this date. Data refreshes daily at 06:00 AM IST. " +
    ...
    if (!record.scraped?.[lang]) {
      return res.status(200).json({
        ...
        warning: `Language "${lang}" not yet scraped for this date. Showing English. ` +
                 `Trigger: POST /api/horoscope/scrape/${sign}?lang=${lang}`,
      });
    }

    return res.status(200).json({ success: true, found: true, data: formatRecord(record, lang) });
  } catch (err) {
...
export const getAllSignsHoroscope = async (req, res) => {
  ...
  if (dateParam) {
    targetDate = new Date(dateParam + "T00:00:00.000Z");
    if (isNaN(targetDate)) return res.status(400).json({ success: false, message: "Invalid date." });
  } else {
    targetDate = todayUTC();
  }
  ...
  try {
    ...
    return res.status(200).json({
      success: true, date: toDateString(targetDate), language: lang,
      total: records.length, data: records.map((r) => formatRecord(r, lang)),
    });
  } catch (err) {
...
export const scrapeAndSaveSingleSign = async (req, res) => {
  ...
  try {
    ...
    if (lang && SUPPORTED_LANGS.includes(lang)) {
      ...
      console.log(`[Controller] Re-scraping ${lang.toUpperCase()} for: ${sign}`);
      ...
      const ratingKeys = ["health","wealth","family","loveMatters","occupation","marriedLife"];
      if (scraped.ratings) {
        ratingKeys.forEach((k) => {
          if (scraped.ratings[k] !== undefined) setData[`ratings.${k}.${lang}`] = scraped.ratings[k];
        });
      }
      if (lang === "en") {
    ...
    return res.status(200).json({
      ...
      scraped: scraped.scraped,
      errors:  Object.keys(scraped.errors || {}).length ? scraped.errors : undefined,
      data:    formatRecord(record, "en"),
    });
  } catch (err) {
    console.error(`scrapeAndSaveSingleSign error (${sign}):`, err.message);
    return res.status(502).json({ success: false, message: err.message || "Scraping failed" });
  }
};
...
export const scrapeAndSaveAllSigns = async (req, res) => {
  try {
    ...
    if (!results.length) {
      return res.status(502).json({ success: false, message: "All signs failed.", failures });
    }

    const bulkOps = results.map((s) => ({
      updateOne: { filter: { sign: s.sign, date: today },
                   update: { $set: buildSetPayload(s, today, dateString) }, upsert: true },
    }));
...
export const getHoroscopeHistory = async (req, res) => {
  ...
  if (!SIGNS.includes(sign)) return res.status(400).json({ success: false, message: `Invalid sign: "${sign}"` });
  if (!SUPPORTED_LANGS.includes(lang)) return res.status(400).json({ success: false, message: "Use en, hi, or mr" });

  try {
    const from = new Date(todayUTC());
    from.setUTCDate(from.getUTCDate() - (days - 1));
    const records = await DailyHoroscope.find({ sign, date: { $gte: from } }).sort({ date: -1 });
    return res.status(200).json({
      success: true, sign, language: lang, days, total: records.length,
...
export const getScrapeCoverage = async (req, res) => {
  ...
  try {
    const records = await DailyHoroscope
      .find({ date: targetDate }, { sign: 1, scraped: 1 })
      .sort({ sign: 1 });
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        byLang,
        signs:  records.map((r) => ({ sign: r.sign, en: !!r.scraped?.en, hi: !!r.scraped?.hi, mr: !!r.scraped?.mr })),
      },
    });
  } catch (err) {
...
export const getSupportedSigns = (_req, res) =>
  res.status(200).json({ success: true, total: SIGNS.length, signs: SIGNS, languages: SUPPORTED_LANGS });
